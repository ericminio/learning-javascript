...
const { isFrameFinal, isText, decodeLength, extractMask, extractData, decode, encode } = require('./websocket-frames.js');

describe.only('decoding of small text in one frame', () => {
...
describe.only('encoding of small text in one frame', () => {

    it('does not require a mask', () => {
        expect(encode('a')).to.deep.equal(Buffer.from([0x81, 1, 97]));
    });
});