...
const extractFields = (lines) => {
    let contentDispositionIndex = lines.findIndex(
        (line) => line.indexOf(CONTENT_DISPOSITION) == 0
    );
    let contentDisposition = lines[contentDispositionIndex];
    let nameStartIndex =
        contentDisposition.indexOf(NAME_FIELD) + NAME_FIELD.length;
    ...
    let indexOfEmptyLineAfterHeader = lines.findIndex(
        (line, index) => index > contentDispositionIndex && line.length == 0
    );
    let value = lines
        .slice(indexOfEmptyLineAfterHeader + 1)
        .join(EOL)
        .trim();
...
const maybeAddFileName = (fields, contentDisposition) => {
    if (contentDisposition.includes(FILENAME_FIELD)) {
        ...
        let fileName = contentDisposition.substring(
            fileNameStartIndex + FILENAME_FIELD.length
        );
        fields.fileName = unquote(fileName);
    }
};
const maybeAddContentType = (fields, lines) => {
    let contentTypeIndex = lines.findIndex(
...
const create = (options) => {
    ...
    let payload = `${boundary}${EOL}${segments.join(
        `${boundary}${EOL}`
    )}${boundary}--${EOL}`;
