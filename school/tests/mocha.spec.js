...
describe('Mocha', function () {
    ...
    it('exposes test name in this.ctx.test.title with arrow function', () => {
        expect(this.ctx.test.title).to.equal(
            'exposes test name in this.ctx.test.title with arrow function'
        );
    });
    it('exposes test name in this.test.title with regular function', function () {
        expect(this.test.title).to.equal(
            'exposes test name in this.test.title with regular function'
        );
    });
    describe(`
        multiline
        ...
        it('is exposed in this.title', () => {
            expect(title).to.equal(
                'multiline context with regular function callback'
            );
        });
    });
    ...
    describe('with promises', () => {
        ...
        it('can expect rejection', () => {
            return Promise.reject(66).catch((value) => {
                expect(value).to.equal(66);
            });
        });
    });
    ...
    describe('with chai-as-promised', () => {
        ...
        it('can expect rejection', () => {
            return expect(Promise.reject(66)).to.be.rejected.then((error) => {
                expect(error).to.equal(66);
            });
        });
        it('can expect rejected object field', () => {
            return expect(
                Promise.reject({ value: 66 })
            ).to.eventually.be.rejected.and.have.property('value', 66);
        });
        it('can expect rejected object', () => {
            return expect(Promise.reject({ value: 66 })).to.be.rejected.then(
                (error) => {
                    expect(error).to.deep.equal({ value: 66 });
                }
            );
        });
    });

    describe('dynamic tests creation', () => {
        [
            { a: 1, b: 2, expected: 3 },
            { a: 1, b: 20, expected: 21 },
        ].forEach(({ a, b, expected }) => {
            it(`${a} + ${b} = ${expected}`, () => {
                expect(sut(a, b)).to.equal(expected);
            });
        });
