...
describe('Date', function () {
    ...
    describe('adding one day', () => {
        it('can be done adding ms', () => {
            ...
            expect(tomorrow.getTime()).to.equal(
                new Date(2017, 7, 26).getTime()
            );
        });
    });
    ...
    describe('finding Monday around a date', () => {
        ...
        it('is straigth-forward from Monday to Saturday', () => {
            ...
            let monday = new Date(
                thursday.getTime() - dayLength * (thursday.getDay() - 1)
            );
        ...
        const f = (n) => (n + 6) % 7;
        const mondayOf = (date) =>
            new Date(date.getTime() - dayLength * f(date.getDay()));
        ...
        it('can be unified', () => {
            ...
            let thursday = new Date(2015, 0, 15);
            let sunday = new Date(2015, 0, 18);
            expect(mondayOf(thursday).getTime()).to.equal(
                new Date(2015, 0, 12).getTime()
            );
            expect(mondayOf(sunday).getTime()).to.equal(
                new Date(2015, 0, 12).getTime()
            );
        });

        const addDays = (count, date) =>
            new Date(date.getTime() + dayLength * count);
